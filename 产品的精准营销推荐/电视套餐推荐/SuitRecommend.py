...
class TVRecommend:
    ...
    def __init__(self, limit=1000, source_type='sql', file_name='suit_for_analysis.csv', encoding='GBK',
                 recommend_count=30,
                 train_rate=1.0, test_rate=0, column_name=('用户号', '销售品'), quiet=False):
        self.limit = limit
    ...
    # 读取数据
    def read_data(self, source_type='sql', file_name='suit_for_analysis.csv', encoding='GBK'):
        if source_type == 'sql':
    ...
    def generate_score_matrix(self):
        ...
        for i in range(len(pre_data)):
            p = pre_data.loc[i, [self.column_name[0], self.column_name[1]]]
    ...
    def test(self):
        ...
        if self.test_df is None:
            df = pd.DataFrame({}, columns=['K', '召回率', '准确率', '推荐覆盖率'])
        else:
            df = self.test_df
